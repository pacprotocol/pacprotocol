...
void GovernanceList::Vote(uint256 nHash, vote_outcome_enum_t eVoteOutcome)
{
    ...
    if (eVoteSignal == VOTE_SIGNAL_NONE || eVoteOutcome == VOTE_OUTCOME_NONE) {
        LogPrint(BCLog::GOBJECT, "We are not going to be voting.");
        return;
    }
    ...
    mnList.ForEachMN(true, [&](const CDeterministicMNCPtr& dmn) {
        CKey votingKey;
        if (m_wallet->GetKey(dmn->pdmnState->keyIDVoting, votingKey)) {
            nTotalVotes++;
            votingKeys.emplace(dmn->proTxHash, votingKey);
        }
    });

    LogPrint(BCLog::GOBJECT, "Will be voting using %d deterministic masternodes.", nTotalVotes);
    ...
    LogPrint(BCLog::GOBJECT, "Voted successfully %d time(s) and failed %d time(s).", nSuccessful, nFailed);
}
