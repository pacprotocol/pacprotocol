 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Dash (https://www.pacprotocol.org/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\nUsage:\n"
                  "  pacprotocold [options]                     " + strprintf("Start %s Daemon", PACKAGE_NAME) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see pacprotocold -h for a list of options.\n", argv[i]);
                return false;
            }
        }
...
#endif
            fprintf(stdout, "pacprotocol server starting\n");
